...
**为什么要学习类？** 类让代码更整洁、可复用。比如模拟无线通信时，一个类就能管理所有相关数据和计算，避免乱七八糟的变量。
...
- **本质原因**：
  - **值类 vs 句柄类**：
    ...
    | 类型            | 继承方式      | 修改行为                                                     | 
    | --------------- | ------------- | ------------------------------------------------------------ | 
    | **值类** (默认) | 无 `< handle` | 方法内 `obj` 是**拷贝**（本地副本）。修改只影响副本，原对象不变。除非返回新对象。 | 
    | **句柄类**      | `< handle`    | 方法内 `obj` 是**引用**（指针）。修改直接影响原对象，无需返回。 | 
  - 在值类中，调用 `transmit(obj);` 时，MATLAB 传的是拷贝。如果不返回，将不会改变原 `tx` 值！
...
- **注意**：句柄类对象所有引用共享变化（e.g., `tx2 = tx; tx.transmit();` → tx2 也变）。
